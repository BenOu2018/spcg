ALTER TABLE levels
  ADD COLUMN IF NOT EXISTS statement_assets JSONB NOT NULL DEFAULT '[]'::jsonb;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'levels_statement_assets_array'
  ) THEN
    ALTER TABLE levels
      ADD CONSTRAINT levels_statement_assets_array CHECK (
